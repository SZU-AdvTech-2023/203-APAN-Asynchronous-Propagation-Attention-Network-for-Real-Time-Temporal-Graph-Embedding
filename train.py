...
def train(args, logger):
    ...
    logger.info('Model training with {}'.format(device_string))
    ...
    for epoch in range(args.n_epoch):
        ...
        if early_stopper.early_stop_check(early_stopper_metric):
            ...
            logger.info('Best model on Test {} Task | ap: {:.4f} | auc: {:.4f} | acc: {:.4f} | Loss: {:.4f}'.format(args.tasks, test_ap, test_auc, test_acc, test_loss))
    ...
    # 如果达到了epoch上限则记录当前test的最佳模型，以及准确率
    logger.info('Best Model is epoch {}'.format(early_stopper.best_epoch))
    logger.info('Best Model on {} Task | ap: {:.4f} | auc: {:.4f} | acc: {:.4f} | Loss: {:.4f}'.format(args.tasks, early_stopper.best_ap, early_stopper.best_auc,  early_stopper.best_acc, test_loss))
