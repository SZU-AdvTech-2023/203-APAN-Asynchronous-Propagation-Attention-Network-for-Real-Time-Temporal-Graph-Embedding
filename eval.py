from torchmetrics.functional import accuracy, auroc, average_precision, roc
import torch
...
def eval_epoch(args, logger, g, dataloader, encoder, decoder, msg2mail, loss_fcn, device, num_samples):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (input_nodes, pos_graph, neg_graph, blocks, frontier, current_ts) in enumerate(dataloader):
            ...
            if 'LP' in args.tasks:
                pred = logits > 0.5
                m_ap.append(average_precision(logits, labels, task='binary').cpu().numpy())
                m_auc.append(auroc(logits, labels, task='binary').cpu().numpy())
                m_acc.append(accuracy(pred, labels, task='binary').cpu().numpy())
            else:
    ...
    if 'LP' in args.tasks:
        ...
        ap = average_precision(logits_all, labels_all, task='binary').cpu().item()
        auc = auroc(logits_all, labels_all, task='binary').cpu().item()
        acc = accuracy(pred_all, labels_all, task='binary').cpu().item()
    ...
    logger.info('Inference time :%s',np.sum(m_infer_time))
